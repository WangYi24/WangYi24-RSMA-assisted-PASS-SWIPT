function [W, SINR_hist] = beamforming_L_v3(h, sigma2, Pmax, opts)
% 之前内层求w时判断收敛的条件好像有问题，我直接用的mu，应该用对偶问题的目标函数
% 用对偶问题的目标函数作为收敛条件时，无法获得一个较好的解，并且迭代次数很多。这是为什么？
...
% ==== Step 2: 固定 y，更新 W ====
    ...
    for lambda_iter = 1:maxlambda
        ...
        % 计算对偶目标函数L
        L = compute_L(lambda, mu, y, h, Pmax, sigma2);
        L_hist = [L_hist,L];
        % if L>L_max            
        %     if (L-L_max)<1e-6
        %         break;
...
function L = compute_L(lambda, mu, y, h, Pmax, sigma2)
    [N,M]=size(h);
    b = zeros(N,M);
    S = cell(M,1);
    term2 = 0;
    for m = 1:M
        b(:,m) = lambda(m) * y(m) * h(:,m);

        weights = lambda.* abs(y).^2;
        S{m} = h * diag(weights) * h' - lambda(m).* abs(y(m)).^2*h(:,m)*h(:,m)';

        term2 = term2 + b(:,m)' / (S{m}+mu*eye(N)) * b(:,m);
    end

    term1 = sum(lambda.*abs(y).^2*sigma2);
    term2 = -term2;
    term3 = -mu * Pmax;
    L = term1 + term2 + term3;
end
