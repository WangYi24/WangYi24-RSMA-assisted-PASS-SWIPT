...
K = 2;
J = 1;
E_range = [0.3:0.02:0.5]*1e-6;
% E_range = [0.5:0.02:0.6]*1e-6;
...
for iE = 1:length(E_range)
    ...
    parfor it = 1:ct %Monte Carlo simulations   
        ...
        if SumRate_best==-inf
            ...
            c_ct(:,it) = log2(1+term_c./sum(G, 2) + sigma2);
            R_p = zeros(K,1);
            for k = 1:K
    ...
    filename = sprintf("E_Conv[%.2f]221_D%d_P%d.mat",E_range(iE)*1e6,D_x,P);
    save(filename)
end
figure
...
%     filename = sprintf("P_Conv[%d]221_D%d.mat",P_range(iP),D_x);
%     save(filename)
% end
% 
% figure
